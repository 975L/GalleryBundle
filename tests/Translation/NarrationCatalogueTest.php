...
namespace c975L\GalleryBundle\Tests\Translation;
...
{
    // The bundle's own narrations, resolved in its "gallery" domain suffixed "_narration" - spoken and never drawn, hence the two locales the rest of the bundle does not stop at
    ...
    public function testEveryDeclaredNarrationIsShippedInBothLocales(): void
    {
        ...
        foreach (self::LOCALES as $locale) {
            ...
            foreach ($declared as $file => $keys) {
                foreach ($keys as $key) {
                    $this->assertContains($key, $catalogue, sprintf('"%s" declares "%s", which "gallery_narration.%s.xlf" does not ship.', $file, $key, $locale));
                }
            }
        }
    }
    ...
    public function testNoNarrationIsShippedWithoutBeingDeclared(): void
    {
        ...
        foreach ($this->catalogue('en') as $key) {
            $this->assertContains($key, $declared, sprintf('"gallery_narration.en.xlf" ships "%s", which nothing in "src/" declares any more.', $key));
        }
    }
    ...
    {
        $path = $this->root() . '/translations/gallery_narration.' . $locale . '.xlf';
