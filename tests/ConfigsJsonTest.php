...
class ConfigsJsonTest extends TestCase
{
    ...
    {
        // Every configs*.json, not just configs.json: ConfigDeclarationLocator globs them all at runtime
        $files = glob(\dirname(__DIR__) . '/config/configs*.json') ?: [];
    ...
    {
        $xliff = simplexml_load_file(\dirname(__DIR__) . '/translations/' . $domain . '.' . $locale . '.xlf');
        $translations = [];
    ...
    // The label shown in the back office is derived from the slug (Config::getLabelTranslationKey()), so a "label" that doesn't match it would be translated on one screen and raw on the next
    public function testLabelsMatchTheKeyDerivedFromTheSlug(): void
    {
        foreach ($this->loadConfigs() as $config) {
            $this->assertSame('label.' . str_replace('-', '_', $config['slug']), $config['label']);
        }
    }

    // Both the label and the description of every entry are translated in each shipped locale, in the domain ConfigCrudController reads them from
    public function testLabelsAndDescriptionsAreTranslatedInEveryLocale(): void
    {
        $configs = $this->loadConfigs();

        foreach (self::LOCALES as $locale) {
            $translations = $this->loadTranslations('site_config', $locale);
    ...
    // The group's own label lives in the "config" domain instead, and this bundle ships its group rather than expecting ConfigBundle to know about it
    public function testEveryGroupIsTranslatedInEveryLocale(): void
    {
        $groups = array_unique(array_column($this->loadConfigs(), 'group'));

        foreach (self::LOCALES as $locale) {
            $translations = $this->loadTranslations('config', $locale);
            foreach ($groups as $group) {
                $this->assertArrayHasKey('label.group_' . $group, $translations, sprintf('Group "%s" has no %s translation', $group, $locale));
            }
        }
    }
}
