...
class GalleryPhotoUserListenerTest extends TestCase
{
    ...
    public function testPrePersistIgnoresEntitiesThatAreNotGalleryPhoto(): void
    {
        ...
        (new GalleryPhotoUserListener($security))->prePersist($this->createPersistArgs(new \stdClass()));
    ...
    public function testPrePersistLeavesUserNullWhenNobodyIsLoggedIn(): void
    {
        ...
        (new GalleryPhotoUserListener($security))->prePersist($this->createPersistArgs($photo));
    ...
    public function testPreUpdateIgnoresEntitiesThatAreNotGalleryPhoto(): void
    {
        ...
        (new GalleryPhotoUserListener($security))->preUpdate($this->createUpdateArgs(new \stdClass(), $entityManager));
    ...
    public function testPreUpdateDoesNotRecomputeChangeSetWhenNobodyIsLoggedIn(): void
    {
        ...
        (new GalleryPhotoUserListener($security))->preUpdate($this->createUpdateArgs($photo, $entityManager));

        $this->assertNull($photo->getUser());
    }

    // Somebody logged in on a creation: the photo simply gets the user, no changeset to recompute since Doctrine has not diffed anything yet
    ...
    // Somebody logged in on an update: the photo's user is overwritten (the listener tracks the last editor, not the uploader) and the changeset is recomputed so Doctrine actually includes "user" in the SQL UPDATE, even when it's the only field that changed
    public function testPreUpdateAssignsTheLoggedInUserAndRecomputesTheChangeSet(): void
    {
        ...
        $classMetadata = new ClassMetadata(GalleryPhoto::class);
        $unitOfWork = $this->createMock(UnitOfWork::class);
        $unitOfWork->expects($this->once())
            ->method('recomputeSingleEntityChangeSet')
            ->with($classMetadata, $this->isInstanceOf(GalleryPhoto::class));

        $entityManager = $this->createMock(EntityManagerInterface::class);
        $entityManager->expects($this->once())->method('getClassMetadata')->with(GalleryPhoto::class)->willReturn($classMetadata);
        $entityManager->method('getUnitOfWork')->willReturn($unitOfWork);

        $photo = (new GalleryPhoto())->setUser($this->createStub(UserInterface::class));

        (new GalleryPhotoUserListener($security))->preUpdate($this->createUpdateArgs($photo, $entityManager));

        $this->assertSame($newUser, $photo->getUser());
    }
}
