...
`composer update`. Both join the page's one Stimulus application, `globalThis.c975lStimulusApp`, whichever
c975L barrel loads first starting it: a barrel of its own would build the `live` controller a second time
and a Live Component would answer twice.
...
- `GalleryCategory` — `slug` (the url segment), `title`, `summarySocialNetwork` (rich text, printed
  ...
  *Masking is not deleting*), `license` (a `Model\GalleryLicense` value, `reserved` by default, see
  *Structured data*), `medias`. Implements `HasBlocksInterface` (its own editorial heading) and
  `TrashableInterface`.
...
`gallery_category` serves its grid `GalleryController::MEDIAS_PER_PAGE` (60) medias at a time, read from
`?p=`: `category.html.twig` receives `medias` as UiBundle's `Pagination` — countable and iterable, plus
`getCurrentPageNumber()`, `getPageCount()`, `getTotalItemCount()`, `route` and `query()` — and UiBundle's
`infinite-scroll.js` appends the next page as the visitor scrolls, into the grid
`<twig:c975LGallery:Gallery:Medias … infinite="true"/>` marks. A page past the last one answers 404.
...
`Lightbox` writes no markup of its own: it names the two files and the words the link is announced with,
and hands them to UiBundle's `<twig:c975LUi:Image:Zoom>` (`c975l/core-bundle` `^1.26`), which carries the
`<dialog>` and its Stimulus controller. What stays this bundle's is the passe-partout, applied on
`.gallery-media-container .image-zoom__image`, and the three `--gallery-lightbox-*` tokens handed to the
zoom's own on `.gallery-media-container`.
...
| highres | `GalleryMedia::HIGHRES_WIDTH` | 2048px | the zoom, fetched only on demand |
...
| `gallery-social-order` | choice `random`/`oldest` | the order SocialBundle's publication takes the photographs in, `random` out of the box |
...
`Email\GalleryEmailTemplateProvider`, `Service\GallerySocialContentSource` (UiBundle's `SocialContentSourceInterface`, source type `gallery_media`: one image photograph at a time for SocialBundle's publication, never the same twice, read off the configured route prefix since the run has no request).
...
the photograph's own url where the page actually prints an offer for it, the terms page otherwise. That terms
page, config `gallery-license-url` (a path made absolute by `UrlHelper` in the extension, or a full url), is also
published as `license` while the gallery keeps all rights reserved; empty, neither is. `GalleryCategory::$license`
offers the Creative Commons 4.0 set and CC0 (`GalleryLicense::all()`, `deedUrl()`, `label()`, `name()`, `icons()`): the deed then
becomes `license`, read off the media's own category rather than the gallery walked through, and is linked by
`Gallery:Credits` in place of the rights-reserved mark, drawn with Creative Commons' icons (`public/icons/cc-*.svg`, CSS masks) and its short name (`CC BY-NC 4.0`), while a reserved one links `gallery-license-url` when set; a category keeping `reserved` has its pages carry
`<meta name="tdm-reservation" content="1">`. An unknown value given to `setLicense()` falls back on `reserved`. The print offer is handed to
...
was served with, numbered from `gallery_json_ld()`'s fourth argument, `$offset` - `(page - 1) * perPage` on a
page after the first; the index is that `ItemList` alone.
