...
offers the Creative Commons 4.0 set and CC0 (`GalleryLicense::all()`, `deedUrl()`, `label()`): the deed then
becomes `license`, read off the media's own category rather than the gallery walked through, and is linked by
`Gallery:Credits` in place of the rights-reserved mark; a category keeping `reserved` has its pages carry
